...
async function main() {
  ...
  if (propertyId) {
    ...
    // Per API.md: updating a property resets its status back to "Pending".
    // Re-approve it so the downstream booking/favorites tests run against
    // an Approved property as intended.
    if (adminToken) {
      const reApprove = await call("PATCH", `/admin/properties/${propertyId}/status`, {
        body: { status: "Approved" },
        token: adminToken,
      });
      logResult("Re-approve Property after Update (Admin)", reApprove.ok, reApprove.status);
    }
  } else {
